...
const work = read('../src/data/work.json')
...
// ── Before and after ──────────────────────────────────────────────────────
// Every job needs both photos generated by `npm run photos`, alt text for each,
// and a trade that exists — the caption and the "ask about a job like this"
// button both read the trade's name from services.json.
const slugs = new Set(services.map((s) => s.slug))
const workManifest = existsSync(at('../src/assets/work/manifest.json'))
  ? read('../src/assets/work/manifest.json')
  : {}

check(work.length > 0, 'work.json is empty')
check(unique(work.map((w) => w.id)), 'work.json has duplicate ids')
for (const job of work) {
  const where = `work.json[${job.id ?? '?'}]`
  // `summary` is optional: the carousel shows only the title. The one-liners
  // are kept in work.json in case a description comes back.
  check(typeof job.title === 'string' && job.title.trim(), `${where}: missing "title"`)
  check(slugs.has(job.trade), `${where}: trade "${job.trade}" is not a slug in services.json`)
  for (const side of ['before', 'after']) {
    check(job[side]?.alt?.trim(), `${where}: ${side} photo needs alt text`)
    const files = [`${job.id}-${side}-sm.webp`, `${job.id}-${side}-lg.webp`]
    if (side === 'after') files.push(`${job.id}-thumb.webp`)
    check(
      workManifest[`${job.id}-${side}`] &&
        files.every((f) => existsSync(at(`../src/assets/work/${f}`))),
      `${where}: ${side} photo not generated — add ${side}${job.id}.jpg to photos/originals/ and run \`npm run photos\``
    )
  }
}
...
console.log(
  `✓ ${services.length} trades, ${faqs.length} FAQs, ${work.length} jobs, ` +
